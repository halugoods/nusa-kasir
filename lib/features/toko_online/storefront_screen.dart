...
class _StorefrontScreenState extends ConsumerState<StorefrontScreen> {
  ...
  void _toggleCartItem(Product p) {
    ...
    if (idx >= 0) {
      if (_cart[idx].qty <= 1) { _cart.removeAt(idx); } else { _cart[idx].qty--; }
    } else {
...
class _ProductCard extends StatelessWidget {
  final Product product; final bool isDark;
  final bool inCart; final int cartQty; final VoidCallback onToggle;
  const _ProductCard({required this.product, required this.isDark, required this.inCart, required this.cartQty, required this.onToggle});

  @override
  Widget build(BuildContext context) {
    final outOfStock = product.stock <= 0;
    final emoji = NusaConfig.catEmojiFor(product.category);
            ...
            child: Stack(children: [
              ClipRRect(
                ...
                child: product.imagePath != null && product.imagePath!.isNotEmpty
                    ? Image.asset(product.imagePath!, fit: BoxFit.cover, width: double.infinity, errorBuilder: (_, __, ___) => _placeholder(isDark, emoji, gradient))
                    : _placeholder(isDark, emoji, gradient),
              ),
              if (outOfStock)
                Positioned(top: 8, left: 8,
                  child: Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                    decoration: BoxDecoration(color: NusaConfig.textPrimary, borderRadius: BorderRadius.circular(6)),
                    child: const Text('HABIS', style: TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.w700)))),
              Positioned(bottom: 6, right: 6,
                child: Container(
                  padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                  decoration: BoxDecoration(color: NusaConfig.surfaceColor, borderRadius: BorderRadius.circular(10), boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: 0.08), blurRadius: 4)]),
          ...
          Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(product.name, maxLines: 2, overflow: TextOverflow.ellipsis,
                style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, height: 1.2, color: outOfStock ? NusaConfig.textTertiary : (isDark ? NusaConfig.darkTextPrimary : NusaConfig.textPrimary))),
              ...
              if (outOfStock)
                ...
                Container(
                  ...
                  child: Row(children: [
                    GestureDetector(onTap: onToggle, child: const SizedBox(width: 32, height: 34, child: Center(child: Icon(Icons.remove, size: 16, color: Colors.white)))),
                    ...
                    GestureDetector(onTap: onToggle, child: const SizedBox(width: 32, height: 34, child: Center(child: Icon(Icons.add, size: 16, color: Colors.white)))),
                  ]),
                )
              else
                SizedBox(
  ...
  Widget _placeholder(bool isDark, String emoji, List<Color> gradient) {
    return Container(
